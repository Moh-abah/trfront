...
//     const areaSeriesRef = useRef<ISeriesApi<"Area"> | null>(null)
//     const currentSeriesRef = useRef<ISeriesApi<any> | null>(null)
//     const liveVolumeDataRef = useRef<any>(null);
...
//             if (showVolume && volumeSeriesRef.current) {
//                 const currentVolume = Number(liveCandle.volume || 0);
//                 // ✅ تحديث بيانات الفوليوم للشمعة الحية
//                 volumeSeriesRef.current.update({
//                     time: candleData.time,
//                     value: Number(liveCandle.volume || 0),
//                     color: candleData.close >= candleData.open
...
//                 // ✅ تحديث الحجم الأقصى إذا كان حجم الشمعة الحية أكبر
//                 if (currentVolume > maxVolumeRef.current) {
//                     maxVolumeRef.current = currentVolume;

//                     // ✅ تحديث مقياس الفوليوم يدوياً
//                     chartRef.current.priceScale("volume").applyOptions({
//                         autoScale: false,
//                         minimum: 0,
//                         maximum: maxVolumeRef.current * 1.8,
//                         scaleMargins: {
//                             top: 0.8,
//                             bottom: 0
//                         },
...
//         try {
//             // ✅ استخدام كل البيانات بما فيها الشمعة الحية
//             const volumeData = allCandles.map(c => {
//                 const time = toUTCTimestamp(c.time);
//                 const value = Number(c.volume || 0);
//                 const color = c.close >= c.open
...
//             // ✅ حساب الحد الأقصى من كل البيانات
//             const volumes = allCandles.map(c => Number(c.volume || 0));
//             const maxVolume = volumes.length > 0
//                 ? Math.max(...volumes, 1000)
//                 : 1000;

//             maxVolumeRef.current = maxVolume;

//             // ✅ إعدادات المقياس الصحيحة:
//             chartRef.current.priceScale("volume").applyOptions({
//                 autoScale: false,
//                 minimum: 0,
//                 maximum: maxVolumeRef.current * 1.5,
//                 scaleMargins: {
//                     top: 0.8,
//                     bottom: 0.05, // ✅ تغيير مهم: إضافة مسافة من الأسفل
//                 },
//             });
