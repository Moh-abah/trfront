...
// // app/services/indicators/smc-order-block-indicator.ts
...
//     // ✅ تخزين الـ Chart بشكل صريح لضمان الوصول الآمن
...
//             // ✅ التعديل الجوهري: تمرير timeScale إلى الـ Primitive
//             const timeScale = this._chart.timeScale();
//             this._primitive = new SMCOrderBlockPrimitive([], [], timeScale);
...
//             // ملاحظة: الباك إند يرسل البيانات داخل meta، ولكن هنا نستقبل IndicatorData التي تحتوي على metadata
...
//             const primitiveOrderBlocks: PrimitiveOrderBlock[] = this.convertOrderBlocks(orderBlocksData);
//             const primitiveSwingPoints: SwingPoint[] = this.convertSwingPoints(swingPointsData);

//             // 🔥 تحديث Primitive بالبيانات الجديدة
//             if (this._primitive) {
//                 // إزالة Debounce للاستجابة الفورية
//                 if (this._updateDebounceTimeout) {
//                     clearTimeout(this._updateDebounceTimeout);
//                 }

//                 // تحديث البيانات مع تسجيل الأداء
...
//                     totalBlocks: primitiveOrderBlocks.length,
//                     totalSwingPoints: primitiveSwingPoints.length
//                 });
//             } else {
//                 console.error('[SMC Indicator] ❌ Primitive not initialized - call createSeries first');
//             }
...
//                 strength: Math.min(Math.max(Number(block.strength || 0.5), 0), 1) // تأكد أن القوة بين 0 و 1
//             };
//         }).filter((item): item is OrderBlock => item !== null);
//     }
...
//                 type: point.type || (point.level > 0 ? 'high' : 'low'), // تخمين منطقي
...
//     /**
//      * الحصول على إحصائيات المؤشر (لأغراض التصحيح والمراقبة)
//      */
//     getStats(): {
