...
//                     <div
//                       key={indicator.name}
//                       className={`group border-b border-border transition-colors duration-150 ${isSelected
//                           ? `bg-card border-l-2 ${CATEGORY_BORDER[category]}`
//                           : 'bg-background border-l-2 border-transparent hover:bg-muted/50'
//                         }`}
//                     >
//                       <div className="flex items-center gap-3 p-3">
//                         <div
...
//                             if (idx !== -1) toggleIndicator(idx);
//                             else addIndicator(indicator.name);
//                           }}
//                           className="cursor-pointer"
//                         >
//                           <div
...
//                           onClick={() => {
//                             if (isSelected) {
//                               setExpandedIndicator(isExpanded ? null : indicator.name);
//                             } else {
//                               addIndicator(indicator.name);
//                               setExpandedIndicator(indicator.name);
//                             }
//                           }}
//                         >
//                           <div className="flex items-center gap-2 mb-0.5">
//                             <span
//                               className={`text-[11px] font-bold tracking-wide uppercase ${isSelected ? 'text-foreground' : 'text-muted-foreground'
//                                 }`}
//                             >
//                               {indicator.display_name}
//                             </span>
...
//                               <Badge
//                                 variant="outline"
//                                 className={`h-4 px-1 text-[8px] border-current bg-transparent font-mono opacity-50 ${CATEGORY_COLORS[indicator.type]
//                                   }`}
//                               >
//                                 {indicator.type}
//                               </Badge>
...
//                             >
//                               {isExpanded ? (
//                                 <div className="h-3 w-3 border-l border-t border-foreground rotate-45 -translate-y-[1px] translate-x-[1px]" />
//                               ) : (
//                                 <div className="h-3 w-3 border-l border-t border-muted-foreground rotate-135 -translate-y-[1px] translate-x-[1px]" />
//                               )}
//                             </Button>
//                           )}
//                         </div>
...
//                                     const idx = selectedIndicators.findIndex(i => i.name === indicator.name);
//                                     if (idx !== -1) {
//                                       updateIndicatorParam(idx, paramName, parseFloat(e.target.value) || 0);
//                                     }
//                                   }}
//                                   className="h-6 w-full text-[10px] bg-transparent border-0 text-right focus:ring-0 p-0 font-mono text-foreground"
//                                 />
...
// // export function IndicatorSelector({ selectedIndicators, onIndicatorsChange, timeframe, onTimeframeChange }: IndicatorSelectorProps) {
// //   // ✅ التغيير الأساسي: استخدام ID للمؤشر المفتوح بدلاً من الاسم
// //   const [expandedIndicatorId, setExpandedIndicatorId] = useState<string | null>(null);
// //   const categories = Object.keys(INDICATORS_BY_CATEGORY);

// //   // ✅ دالة لتوليد ID عشوائي لكل نسخة مؤشر
...
// //     // ✅ إنشاء مثيل جديد يحتوي على ID و Label فريدين
// //     const newIndicator: IndicatorConfig = {
// //       id: generateId(),              // معرف فريد للباك إند
// //       label: metadata.display_name,   // الاسم الظاهر للمستخدم
// //       name: metadata.name,           // الاسم التقني (rsi, sma..)
// //       type: metadata.category,
// //       params: { ...metadata.default_params },
// //       enabled: true,
...
// //     // ✅ إضافة للمصفوفة (يسمح بالتكرار الآن)
// //     onIndicatorsChange([...selectedIndicators, newIndicator]);
// //     // فتح إعدادات المؤشر الجديد تلقائياً
// //     setExpandedIndicatorId(newIndicator.id);
// //   };
...
// //   // ✅ دالة جديدة لتحديث الاسم الظاهر (Label)
// //   const updateIndicatorLabel = (id: string, newLabel: string) => {
// //     const newIndicators = selectedIndicators.map(ind =>
// //       ind.id === id ? { ...ind, label: newLabel } : ind
// //     );
// //     onIndicatorsChange(newIndicators);
// //   };
...
// //   // ✅ لم نعد بحاجة للتحقق من الاسم لأننا نسمح بالتكرار الآن
// //   // لكننا نحتاج للبحث عن المؤشر في القائمة النشطة باستخدام ID
// //   const getSelectedIndicator = (id: string) => {
// //     return selectedIndicators.find(ind => ind.id === id);
// //   };
...
// //               <TabsContent key={category} value={category} className="p-0 m-0 focus:outline-none">
// //                 {INDICATORS_BY_CATEGORY[category]?.map((indicator: IndicatorMetadata) => {
// //                   // ✅ لاحظ: قمنا بإزالة منطق "isIndicatorSelected" من هنا لنسحب المؤشرات مباشرة
// //                   // المؤشرات المضافة ستظهر دائماً كأزرار "+" في المكتبة
...
// //                       {/* ✅ هنا نعرض قائمة المؤشرات النشطة "التابعة" لهذا النوع */}
// //                       {/* هذه طريقة مدمجة لعرض المكتبة + النشط دون الحاجة لتبويب منفصل */}
// //                       {selectedIndicators
// //                         .filter(activeInd => activeInd.name === indicator.name)
// //                         .map((activeInd) => {
// //                           const isExpanded = expandedIndicatorId === activeInd.id;
...
// //                                 <div className="flex-1 min-w-0">
// //                                   {/* ✅ حقل إدخال لتعديل الاسم الظاهر */}
// //                                   <Input
// //                                     value={activeInd.label}
// //                                     onChange={(e) => updateIndicatorLabel(activeInd.id!, e.target.value)}
// //                                     className={`h-5 w-full px-0 text-[11px] bg-transparent border-0 focus:ring-0 p-0 font-bold tracking-wide uppercase ${activeInd.enabled ? 'text-foreground' : 'text-muted-foreground'}`}
// //                                   />
// //                                   <div className="text-[9px] text-muted-foreground font-mono truncate">
